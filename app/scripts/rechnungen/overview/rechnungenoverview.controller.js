...
/**
 */
angular.module('openolitor-admin').controller('RechnungenOverviewController', [
  '$q',
  '$scope',
  '$rootScope',
  '$filter',
  ...
  function (
    ...
    $scope.paymentTypes = lodash.sortBy(
      EnumUtil.asArray(PAYMENT_TYPES),
      function (pt) {
        return pt.title.toLowerCase();
      }
    );

    $scope.rechnungStati = lodash.sortBy(
      EnumUtil.asArray(RECHNUNGSTATUS),
      function (rs) {
    ...
    $scope.navigateToKunde = function (id) {
      ...
      angular.forEach(listKundeIds, function (kundeId) {
        $scope.filteredEntries.push(
          $filter('filter')($scope.allKunden, { id: kundeId }, true)[0]
        );
      });
    ...
    $scope.downloadRechnung = function (rechnung) {
      ...
      FileUtil.downloadGet(
        'rechnungen/' + rechnung.id + '/aktionen/downloadrechnung',
    ...
    $scope.downloadMahnung = function (rechnung, fileId) {
      ...
      FileUtil.downloadGet(
        'rechnungen/' + rechnung.id + '/aktionen/download/' + fileId,
        'Rechnung ' + rechnung.id + ' Mahnung',
        'application/pdf',
        function () {
    ...
    var alleRechnungenStorniertOderBezahlt = function (selectedItems, items) {
      ...
      for (var i = 0; i < length; ++i) {
        ...
        if (
          items[id].status !== RECHNUNGSTATUS.STORNIERT &&
    ...
    $scope.updateChecked = function () {
      var activeCheckboxes = lodash.pickBy(
    ...
    $scope.actions = [
      ...
      },
      {
        ...
        isDisabled: function () {
          return (
            !$scope.checkboxes.checkedAny ||
            alleRechnungenStorniertOderBezahlt(
      ...
      },
      {
        ...
        onExecute: function () {
          return FileUtil.downloadPost(
            'rechnungen/aktionen/downloadrechnungen',
            {
        ...
        isDisabled: function () {
          return (
            !$scope.checkboxes.checkedAny ||
            !hasRechnungDocument($scope.checkboxes.ids, $scope.checkboxes.data)
          );
        },
      },
      {
        ...
        onExecute: function () {
          return FileUtil.downloadPost(
            'rechnungen/aktionen/downloadmahnungen',
            {
              ids: $scope.checkboxes.ids,
              pdfMerge: 'zip',
            }
      ...
      },
      {
        ...
        onExecute: function () {
          return $http
            .post(appConfig.get().API_URL + 'rechnungen/aktionen/verschicken', {
              ids: $scope.checkboxes.ids,
            })
            .then(function () {
      ...
      },
      {
        ...
        onExecute: function () {
          ...
          $scope.attachment = allRechnungDocumentCreated(
            $scope.checkboxes.ids,
            $scope.checkboxes.data
          );
      ...
      },
      {
        ...
        onExecute: function () {
          return $http
            .post(
              appConfig.get().API_URL + 'rechnungen/aktionen/pain_008_001_02',
              {
                ids: $scope.checkboxes.ids,
              }
            )
            .then(function (file) {
              var data = new Blob([file.data], {
    ...
    msgBus.onMsg('EntityModified', $scope, function (event, msg) {
      if (msg.entity === 'Rechnung') {
        ...
        if (rechnung) {
          ...
          var filteredRechnung = lodash.find(
            $scope.filteredEntries,
            function (r) {
              return r.id === msg.data.id;
            }
