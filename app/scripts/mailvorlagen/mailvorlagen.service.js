...
angular.module('openolitor-admin')
  .factory('MailvorlagenService', ['$rootScope', 'MailvorlagenModel', 'msgBus', 'lodash',
  'DataUtil',
    function($rootScope, MailvorlagenModel, msgBus, lodash, DataUtil) {

      var vorlagen;

      var load = function() {
        MailvorlagenModel.query({}, function(result) {
          vorlagen = lodash.groupBy(result, 'templateType');
        });
      };
      load();
      ...
      msgBus.onMsg('EntityCreated', $rootScope, function(event, msg) {
        if (msg.entity === 'ProjektVorlage') {
          var newModel = new MailvorlagenModel(msg.data);
